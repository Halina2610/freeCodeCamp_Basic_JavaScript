...
//Используя нотацию с точкой и квадратными скобками, 
//установите переменную secondTreeна второй элемент в treesсписке от myPlantsобъекта.
const secondTree = myPlants[1].list[1]; 
//83%

// challenge 97
/*Коллекция записей
Вы создаете функцию, которая помогает поддерживать коллекцию музыкальных 
альбомов. Коллекция организована как объект, содержащий несколько альбомов, 
которые также являются объектами. Каждый альбом представлен в коллекции уникальным idименем свойства. 
В каждом объекте альбома есть различные свойства, 
...
Примечание.recordCollection Для тестов используется копия объекта . Вы не должны напрямую изменять recor
dCollectionобъект.*/
// Setup
...
function updateRecords(records, id, prop, value) { 
  ...
  } else if (prop !== "tracks" && value !== "") { //если нети valueне является пустой строкой, назначь
