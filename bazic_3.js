...
который начинается с числа, представленного параметром, startNumи заканчивается числом, представленным параметром 
endNum. Начальное число всегда будет меньше или равно конечному числу. Ваша функция должна использовать рекурсию, 
вызывая саму себя, и не использовать циклы любого вида. Это также должно работать для случаев, когда оба startNumи 
endNumодинаковы.*/
...
function rangeOfNumbers(startNum, endNum) {
  if (endNum < startNum) {
    ...
    const numbers = rangeOfNumbers(startNum, endNum - 1); //немного не понятно почему константу можно назвать только numbers. при другихназваниях тесты не проходят
    numbers.push(endNum);
    return numbers;
  }
}
